...
public class LCMof2NumbersQno20 {
    ...
    static void lcm(int a, int b, int lcm) {
        if (a > b) {
            lcm = a;
        } else {
            lcm = b;
        } // till here the lcm gets the biggest no of the two entered
        while (true) { // here the while loop runs the if statement continuously until the if statement
                       // is true till then
            // it increments the lcm
            if (lcm % a == 0 && lcm % b == 0) { // then once both the remainder is 0, it prints the LCM and breaks out
                                                // of loop.
