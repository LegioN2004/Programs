...
int main() {
  int a = 3;
  int* b =
      &a;  // this means that int* is a pointer which points to the value of
           // integer and its name is b which is equal to the adress of a
  // & --> address of operator , the variables are stored in the ram after
  // running and its adress is given by the following
