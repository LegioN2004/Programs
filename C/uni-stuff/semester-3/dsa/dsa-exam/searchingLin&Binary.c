...
int binarySearch(int arr[], int size, int element) {
  // low will be the first elememt and high will be the last element. keep
  // searching until low <= high
  ...
  while (low <= high) {
    ...
    if (arr[mid] == element) {
      ...
      low = mid + 1;
      // why + 1, because the element we are looking for is not present before
      // the mid index, so we can skip and start after this part
    } else if (element < arr[mid]) {
      high = mid - 1;
      // why - 1, because the element we are looking for is not present in the
      // mid index, so we can skip and start before this part.
    } else {
