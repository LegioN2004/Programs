...
int binarySearch(int arr[], int size, int element) {
  int low = 0, mid, high = size - 1;  // low will be the first elememt and high
  ...
  while (low <= high) {
    ...
    if (arr[mid] == element) {
      ...
      low =
          mid +
          1;  // why + 1, because the element we are looking for is not present
              // in the mid index, so we can skip and start after this part
    } else if (element < arr[mid]) {
      high =
          mid -
          1;  // why - 1, because the element we are looking for is not present
              // in the mid index, so we can skip and start before this part.
    } else {
